...
# This pipeline reads raw sensor data, applies schema definitions, processes it,
...
with DAG(
    ...
    # Task 2: Define the schema for sensor data using the define_schema function
    task_define_schema = PythonOperator(
        task_id="define_schema",
        python_callable=define_schema,
    )

    # Task 3: Process the raw sensor data to clean and structure it using the process_raw_data function
    task_process_raw_data = PythonOperator(
        task_id="process_raw_data",
        python_callable=process_raw_data,
    )

    # Task 4: Read the raw data from the input path using the read_raw_data function
    ...
    # Task 6: Write the transformed data to the output path using the write_data function
    task_write_raw_data = PythonOperator(
        task_id="write_raw_data",
        python_callable=write_data,
    )

    # Set task dependencies to define the sequence of execution for the pipeline
    # The tasks are organized in a way that ensures proper data flow and processing
    (
        task_start_spark
        >> task_read_data
        >> task_define_schema
